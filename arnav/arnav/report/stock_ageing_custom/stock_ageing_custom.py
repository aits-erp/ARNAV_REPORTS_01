...
def execute(filters: Filters = None) -> tuple:
    # Handle case when filters is None or empty
    if not filters:
        filters = frappe._dict({})
...
def get_chart_data(data: list, filters: Filters) -> dict:
    ...
    # Determine the index for Average Age (depends on columns)
    # Base columns: Item Code, Item Name, Description, Item Group, Brand
    # + Warehouse (if enabled) + Batch No + Available Qty
    avg_age_index = 7  # Default without warehouse
    if filters.get("show_warehouse_wise_stock"):
        avg_age_index = 8  # With warehouse column
    
    data.sort(key=lambda row: row[avg_age_index], reverse=True)
    ...
    for row in data:
        labels.append(row[0])
        datapoints.append(row[avg_age_index])
...
class FIFOSlots:
    ...
    def __compute_incoming_stock(self, row: dict, fifo_queue: list, transfer_key: tuple, serial_nos: list):
        ...
        if transfer_data:
            ...
            valuation = row.stock_value_difference / row.actual_qty if row.actual_qty != 0 else 0
    ...
    def __compute_outgoing_stock(self, row: dict, fifo_queue: list, transfer_key: tuple, serial_nos: list):
        ...
        if serial_nos:
            fifo_queue[:] = [slot for slot in fifo_queue if slot[0] not in serial_nos]
            return
    ...
    def __update_balances(self, row: dict, key: tuple | str):
        ...
        if hasattr(row, 'batch_no') and row.batch_no:
    ...
    def __get_item_query(self):
        ...
        if self.filters.get("item_code"):
            item = item.where(item_table.name == self.filters.get("item_code"))
